                    ...
                    using (var conn = this.factory.GetConnection())
                    {
                        try
                        {
                            string sql = $@"SELECT * FROM Track
                                        WHERE TrackTitle LIKE '%{keyword}%'
                                        OR Artists LIKE '%{keyword}%'
                                        OR AlbumTitle LIKE '%{keyword}%'
                                        OR AlbumArtists LIKE '%{keyword}%'
                                        OR Genres LIKE '%{keyword}%'
                                        OR FileName LIKE '%{keyword}%'";
                            tracks = conn.Query<Track>(sql).ToList();
                        }
                        catch (Exception ex)
                        {
                            LogClient.Error("Could not search tracks. Exception: {0}", ex.Message);
                        }
                    }
                }
                catch (Exception ex)
